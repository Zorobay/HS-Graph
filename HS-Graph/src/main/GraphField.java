...
public class GraphField {
	...
	static List<Component> arrayOfComponents;
	...
	private static void drawLine(double x) {
		getGC2D().setStroke(Color.GREEN);
		...
		for(int i = 0; i < arrayOfComponents.size(); i++){ //Add value of all components for x-value i to get FIRST y value
			fstY += arrayOfComponents.get(i).getValue(fstX);
			System.out.println("x: " + fstX + " y: " + fstY); //For debug purpose
		}
		
		for(int i = 0; i < arrayOfComponents.size(); i++){ //Add value of all components for x-value i to get SECOND y value
			sndY += arrayOfComponents.get(i).getValue(sndX);
		}
	...
	public static void setArrayOfComponents(List<Component> arrayOfComponents){
		GraphField.arrayOfComponents = arrayOfComponents;
	}
}
